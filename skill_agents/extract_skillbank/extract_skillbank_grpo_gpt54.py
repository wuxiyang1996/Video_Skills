...
persistent Skill Bank — using the **skill_agents** pipeline.

This script mirrors ``extract_skillbank_gpt54.py`` but strictly routes
every functional stage through ``skill_agents`` instead of the
...
Usage (from Game-AI-Agent root):
    ...
    # All games
    python -m skill_agents.extract_skillbank.extract_skillbank_grpo_gpt54

    # Specific games
    python -m skill_agents.extract_skillbank.extract_skillbank_grpo_gpt54 --games tetris super_mario

    # Quick test: one episode per game
    python -m skill_agents.extract_skillbank.extract_skillbank_grpo_gpt54 --one_per_game -v

    # Custom input/output dirs
    python -m skill_agents.extract_skillbank.extract_skillbank_grpo_gpt54 \\
        --input_dir labeling/output/gpt54 \\
        --output_dir skill_agents/extract_skillbank/output/gpt54_skillbank_grpo

    # Re-segment against seeded bank
    python -m skill_agents.extract_skillbank.extract_skillbank_grpo_gpt54 --resegment --one_per_game
"""
...
# Path setup
# ---------------------------------------------------------------------------
SCRIPT_DIR = Path(__file__).resolve().parent          # .../skill_agents/extract_skillbank/
GRPO_ROOT = SCRIPT_DIR.parent                         # .../skill_agents/
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract skills from labeled GPT-5.4 rollouts using skill_agents pipeline",
        ...
        epilog=(
            ...
            "  python -m skill_agents.extract_skillbank.extract_skillbank_grpo_gpt54 --one_per_game -v\n\n"
            "  # Custom dirs\n"
            "  python -m skill_agents.extract_skillbank.extract_skillbank_grpo_gpt54 --input_dir path/to/labeled --output_dir path/to/output\n"
        ),
    )
    parser.add_argument(
    ...
    print("=" * 78)
    print("  Skill Bank Extraction (skill_agents pipeline)")
    print("=" * 78)
    ...
    for game, gfiles in sorted(game_files.items()):
        ...
        print(f"\n{'━' * 78}")
        print(f"  GAME: {game} ({len(episode_files)} episodes) [skill_agents]")
        print(f"{'━' * 78}")
    ...
    if not args.skip_archetypes and len(all_catalogs) >= 1:
        print(f"\n{'━' * 78}")
        print(f"  Cross-Game Skill Archetype Aggregation (skill_agents)")
        print(f"{'━' * 78}")
    ...
    # ── Final summary ──
    print(f"\n{'=' * 78}")
    print("  SKILL BANK EXTRACTION COMPLETE (skill_agents)")
    print(f"{'=' * 78}")
