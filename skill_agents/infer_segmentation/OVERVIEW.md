...
| **infer_and_segment(episode, skill_names, env_name, config, preference_store, …)** | **Full pipeline with LLM.** (1) Stage 1 → boundaries C. (2) If store empty: collect segment (+ optional transition) preferences via LLM. (3) Train scorer, decode. (4) For several iterations: collect uncertain preferences, retrain, re-decode. (5) Convert result to SubTask_Experience list. Returns **(SegmentationResult, list[SubTask_Experience], PreferenceStore)**. |
| **infer_and_segment_offline(episode, skill_names, env_name, …, behavior_fit_fn, transition_fn, …)** | **Offline API.** No LLM. Stage 1 → decode with provided scoring functions → SubTask_Experience list. For testing without API. |
