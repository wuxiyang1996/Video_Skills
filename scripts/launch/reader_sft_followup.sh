...
TAG=${TAG:-sft_v1}
jid=$(DATA=$R/sft_v1.jsonl OUT=$R/$TAG EPOCHS=1 sbatch --parsable scripts/launch/reader_sft.sbatch); echo "$(date +%m-%d\ %H:%M) SFT job $jid"
while [ -n "$(squeue -h -j $jid -o %i 2>/dev/null)" ]; do sleep 300; done
if [ ! -f $R/$TAG/adapter/adapter_config.json ]; then echo "$(date +%m-%d\ %H:%M) SFT produced no adapter; tail of err:"; tail -n 5 $R/slurm_logs/sft-$jid.err | cut -c1-200; exit 1; fi
echo "$(date +%m-%d\ %H:%M) adapter ready: $R/$TAG/adapter"; tail -n 3 $R/slurm_logs/sft-$jid.out | cut -c1-200
jid2=$(ADAPTER=$R/$TAG/adapter L1GLOB="$P/narr_px_plus/stages/*/04_l1_example.json" IDS=$M/fresh_ids_300.txt OUT=$M/rdr_${TAG}_rationale_fresh300 sbatch --parsable scripts/launch/reader_eval.sbatch); echo "$(date +%m-%d\ %H:%M) eval job $jid2"
