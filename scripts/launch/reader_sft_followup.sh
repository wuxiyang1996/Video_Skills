...
TAG=${TAG:-sft_v1}
jid=$(DATA=$R/sft_v1.jsonl OUT=$R/$TAG EPOCHS=1 LIMIT=${LIMIT:-} MAXLEN=${MAXLEN:-} LR=${LR:-1e-4} sbatch --parsable scripts/launch/reader_sft.sbatch); echo "$(date +%m-%d\ %H:%M) SFT job $jid"
