#!/usr/bin/env bash
# vrbench_subset240 L1 shards on scavenger (behind the train-split L1). Usage: launch_vrbench_subset240.sh [shard ...]
set -euo pipefail; cd /fs/gamma-projects/vlm-robot/Video_Skills; P=12
SHARDS=("$@"); [ ${#SHARDS[@]} -gt 0 ] || SHARDS=($(seq 0 -1))
for s in "${SHARDS[@]}"; do
  jid=$(sbatch --parsable --job-name=960-$s --partition=scavenger --account=scavenger --qos=scavenger \
