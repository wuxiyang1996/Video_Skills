#!/usr/bin/env bash
# vrbench_subset240 L1 shards on scavenger. Usage: launch_vrbench_subset240.sh [shard ...]
set -euo pipefail; cd /fs/gamma-projects/vlm-robot/Video_Skills; P=/fs/nexus-scratch/wuxiyang/vrbench_pilot_v1/subset240; mkdir -p $P/slurm_logs
SHARDS=("$@"); [ ${#SHARDS[@]} -gt 0 ] || SHARDS=($(seq 0 11))
for s in "${SHARDS[@]}"; do
  jid=$(sbatch --parsable --job-name=vrbs-$s --partition=scavenger --account=scavenger --qos=scavenger \
    ...
    --export=ALL,DATASET=vrbench,SPLIT=train,START_INDEX=0,LIMIT=960,SMOKE=0,PILOT_TAG=vrbench_subset240/shard$s,SERVE_BACKEND=vllm,CLIP_WORKERS=16,GRAPH_WORKERS=2,CLIP_TIMEOUT_S=300,GRAPH_MODEL=openai/gpt-oss-120b,UNIQUE_VIDEOS=1,RETRY_FAILED_CLIP_SCHEMAS=1,EXAMPLE_ID_ALLOWLIST=$P/allowlists/shard$s.txt \
