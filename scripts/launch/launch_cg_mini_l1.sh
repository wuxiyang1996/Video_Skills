#!/usr/bin/env bash
# cg_mini_l1 L1 shards on scavenger (behind the train-split L1). Usage: launch_cg_mini_l1.sh [shard ...]
set -euo pipefail; cd /fs/gamma-projects/vlm-robot/Video_Skills; P=16
SHARDS=("$@"); [ ${#SHARDS[@]} -gt 0 ] || SHARDS=($(seq 0 -1))
for s in "${SHARDS[@]}"; do
  jid=$(sbatch --parsable --job-name=1200-$s --partition=scavenger --account=scavenger --qos=scavenger \
    ...
    --export=ALL,DATASET=cg_bench,SPLIT=train,START_INDEX=0,LIMIT=960,SMOKE=0,PILOT_TAG=cg_mini_l1/shard$s,SERVE_BACKEND=vllm,CLIP_WORKERS=16,GRAPH_WORKERS=2,CLIP_TIMEOUT_S=300,GRAPH_MODEL=openai/gpt-oss-120b,UNIQUE_VIDEOS=1,RETRY_FAILED_CLIP_SCHEMAS=1,EXAMPLE_ID_ALLOWLIST=$P/allowlists/shard$s.txt \
