#!/usr/bin/env bash
# Video-Holmes TRAIN split L1 on scavenger only (gamma is reserved for TIR). Usage: launch_vh_train_l1.sh [shard ...]
set -euo pipefail; cd /fs/gamma-projects/vlm-robot/Video_Skills; P=/fs/nexus-scratch/wuxiyang/vh_train_l1
SHARDS=("$@"); [ ${#SHARDS[@]} -gt 0 ] || SHARDS=(0 1 2 3 4 5 6 7)
for s in "${SHARDS[@]}"; do
  jid=$(sbatch --parsable --job-name=vhtr-$s --partition=scavenger --account=scavenger --qos=scavenger \
    ...
    --export=ALL,DATASET=video_holmes,SPLIT=train,START_INDEX=0,LIMIT=300,SMOKE=0,PILOT_TAG=vh_train_l1/shard$s,SERVE_BACKEND=vllm,CLIP_WORKERS=8,GRAPH_WORKERS=2,CLIP_TIMEOUT_S=300,GRAPH_MODEL=openai/gpt-oss-120b,UNIQUE_VIDEOS=1,RETRY_FAILED_CLIP_SCHEMAS=1,EXAMPLE_ID_ALLOWLIST=$P/allowlists/shard$s.txt \
    scripts/sft_pilot/run_local_qwen_worker.sh)
  echo "vhtr-$s (scavenger) -> $jid"
done
