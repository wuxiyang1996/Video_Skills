...
#
#  2048 game profile:
#    - 4 string actions (up, down, left, right)
#    - 200 max steps/episode
#    - Sparse rewards (many 0-reward steps from non-merge moves)
#    - Goal: reach the 2048 tile
#
#  Usage:
#    conda activate game-ai-agent
#    bash scripts/infer_2048.sh
#
#    # With overrides:
#    EPISODES=16 bash scripts/infer_2048.sh
#    EVAL_GPUS=1 bash scripts/infer_2048.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/output/infer_2048_${TIMESTAMP}}"
mkdir -p "${OUTPUT_DIR}"
...
cleanup() {
    echo ""
    echo "[infer_2048] Shutting down..."
    if [ -n "${VLLM_PID}" ] && kill -0 "${VLLM_PID}" 2>/dev/null; then
        kill "${VLLM_PID}" 2>/dev/null || true
        wait "${VLLM_PID}" 2>/dev/null || true
    fi
    echo "[infer_2048] Done."
}
...
# ── Launch vLLM server ───────────────────────────────────────────────
if [ "${NO_SERVER}" = "0" ]; then
    echo "[infer_2048] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[infer_2048] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[infer_2048] ERROR: vLLM server exited unexpectedly."
            exit 1
        fi
        sleep 5
        WAITED=$((WAITED + 5))
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[infer_2048] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[infer_2048] Using existing vLLM server at ${VLLM_BASE_URL}"
fi
...
if [ ${EXIT_CODE} -eq 0 ]; then
    echo "  2048 inference COMPLETE"
else
    echo "  2048 inference FAILED (exit code ${EXIT_CODE})"
fi
echo "  Output: ${OUTPUT_DIR}"
echo "══════════════════════════════════════════════════════════════"
