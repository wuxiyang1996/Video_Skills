#!/usr/bin/env bash
# ======================================================================
#  Inference: Super Mario with Qwen3-8B  (8 episodes)
#
#  Launches a vLLM server for Qwen/Qwen3-8B and runs 8 inference
#  episodes on Super Mario using the evaluation runner.
#
...
#    - Single-player NES platformer via Orak subprocess env
#    - 7 discrete actions (noop, right, right+A, A, left, left+A, down)
#    - 500 max steps/episode
...
#    bash scripts/infer_super_mario.sh
#
#    # With overrides:
#    EPISODES=16 bash scripts/infer_super_mario.sh
#    EVAL_GPUS=1 bash scripts/infer_super_mario.sh
#    NO_SERVER=1 bash scripts/infer_super_mario.sh
# ======================================================================
set -euo pipefail
...
if [ -z "${DISPLAY:-}" ]; then
    if command -v Xvfb &>/dev/null; then
        ...
        if ! pgrep -f "Xvfb ${XVFB_DISPLAY}" &>/dev/null; then
            echo "[infer_mario] Starting Xvfb on ${XVFB_DISPLAY}..."
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/output/infer_super_mario_${TIMESTAMP}}"
mkdir -p "${OUTPUT_DIR}"
...
cleanup() {
    echo ""
    echo "[infer_mario] Shutting down..."
    if [ -n "${VLLM_PID}" ] && kill -0 "${VLLM_PID}" 2>/dev/null; then
        kill "${VLLM_PID}" 2>/dev/null || true
        wait "${VLLM_PID}" 2>/dev/null || true
    fi
    echo "[infer_mario] Done."
}
trap cleanup EXIT INT TERM

# ── Print banner ─────────────────────────────────────────────────────
echo "══════════════════════════════════════════════════════════════"
echo "  Super Mario Inference: Qwen3-8B"
echo "══════════════════════════════════════════════════════════════"
echo "  Model:          ${MODEL}"
...
# ── Launch vLLM server ───────────────────────────────────────────────
if [ "${NO_SERVER}" = "0" ]; then
    echo "[infer_mario] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[infer_mario] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[infer_mario] ERROR: vLLM server exited unexpectedly."
            exit 1
        fi
        sleep 5
        WAITED=$((WAITED + 5))
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[infer_mario] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[infer_mario] Using existing vLLM server at ${VLLM_BASE_URL}"
fi
...
if [ ${EXIT_CODE} -eq 0 ]; then
    echo "  Super Mario inference COMPLETE"
else
    echo "  Super Mario inference FAILED (exit code ${EXIT_CODE})"
fi
echo "  Output: ${OUTPUT_DIR}"
echo "══════════════════════════════════════════════════════════════"
