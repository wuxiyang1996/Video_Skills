#!/usr/bin/env bash
# ======================================================================
#  Ablation: SFT Decision Agent (No Skill Bank) vs GPT-5.4 on Avalon
#
#  Uses the SFT cold-start LoRA adapter for the decision agent
#  (action_taking) with NO skill bank, evaluated in 5-player Avalon
#  against GPT-5.4 opponents.  8 episodes per player seat (40 total).
#
#  Usage:
#    conda activate game-ai-agent
#    bash ablation_study/run_sft_no_bank_avalon_da.sh
#    EVAL_GPUS=0 bash ablation_study/run_sft_no_bank_avalon_da.sh
#    NO_SERVER=1 VLLM_BASE_URL=http://localhost:8024/v1 bash ablation_study/run_sft_no_bank_avalon_da.sh
# ======================================================================
set -euo pipefail
...
OUTPUT_BASE="${OUTPUT_DIR:-${PROJECT_ROOT}/ablation_study/output/sft_no_bank_avalon_da_${TIMESTAMP}}"
mkdir -p "${OUTPUT_BASE}"
...
cleanup() {
    set +e
    echo ""
    echo "[sft-no-bank-avalon-da] Shutting down..."
    ...
    echo "[sft-no-bank-avalon-da] Done."
}
trap cleanup EXIT INT TERM

if [ ! -f "${SFT_ADAPTER_PATH}/adapter_config.json" ]; then
    echo "[sft-no-bank-avalon-da] ERROR: SFT adapter not found at ${SFT_ADAPTER_PATH}"
...
if [ "${NO_SERVER}" = "0" ]; then
    echo "[sft-no-bank-avalon-da] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    echo "[sft-no-bank-avalon-da] vLLM PID=${VLLM_PID}, log at ${VLLM_LOG}"

    MAX_WAIT=600
    WAITED=0
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[sft-no-bank-avalon-da] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[sft-no-bank-avalon-da] ERROR: vLLM server exited unexpectedly. Check ${VLLM_LOG}"
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[sft-no-bank-avalon-da] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[sft-no-bank-avalon-da] Using existing vLLM server at ${VLLM_BASE_URL}"
...
echo ""
echo "[sft-no-bank-avalon-da] Command:"
...
echo ""
echo "══════════════════════════════════════════════════════════════"
echo "[sft-no-bank-avalon-da] Output: ${OUTPUT_BASE}"
echo "══════════════════════════════════════════════════════════════"
exit ${EXIT_CODE}
