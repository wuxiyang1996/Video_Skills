#!/usr/bin/env bash
# ======================================================================
#  Ablation: SFT-only on Super Mario (SFT LoRA, No Skill Bank)
#
#  Uses the SFT cold-start LoRA adapter for the decision agent
#  (action_taking) with NO skill bank guidance on Super Mario.
#
...
#    bash ablation_study/run_sft_no_skillbank_super_mario.sh
#
#    # With overrides:
#    EVAL_GPUS=0    bash ablation_study/run_sft_no_skillbank_super_mario.sh
#    EPISODES=16    bash ablation_study/run_sft_no_skillbank_super_mario.sh
...
if [ -z "${DISPLAY:-}" ]; then
    if command -v Xvfb &>/dev/null; then
        ...
        if ! pgrep -f "Xvfb ${XVFB_DISPLAY}" &>/dev/null; then
            echo "[sft-mario] Starting Xvfb on ${XVFB_DISPLAY}..."
...
OUTPUT_BASE="${OUTPUT_DIR:-${PROJECT_ROOT}/ablation_study/output/sft_no_skillbank_super_mario_${TIMESTAMP}}"
mkdir -p "${OUTPUT_BASE}"
...
# ── Banner ────────────────────────────────────────────────────────────
echo "══════════════════════════════════════════════════════════════"
echo "  Ablation: SFT-only on Super Mario (SFT LoRA, no bank)"
echo "══════════════════════════════════════════════════════════════"
...
if [ "${NO_SERVER}" = "0" ]; then
    echo "[sft-mario] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    echo "[sft-mario] vLLM PID=${VLLM_PID}, log at ${VLLM_LOG}"

    MAX_WAIT=600
    WAITED=0
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[sft-mario] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[sft-mario] ERROR: vLLM server exited unexpectedly. Check ${VLLM_LOG}"
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[sft-mario] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[sft-mario] Using existing vLLM server at ${VLLM_BASE_URL}"
...
print(f"  Super Mario — SFT-only (SFT LoRA, no bank)")
print(f"    Episodes:    {n}")
...
with open(summary_path, "w") as f:
    json.dump({
        "experiment": "sft_no_skillbank_super_mario",
        "description": "SFT cold-start decision agent (action_taking LoRA) with NO skill bank on Super Mario",
        "results": [{
            "game": "super_mario",
            "sft_no_bank_reward": mean,
            "ci_95": ci,
            "std": std,
...
if [ ${EXIT_CODE} -ne 0 ]; then
    echo "  Super Mario SFT-only inference FAILED (exit code ${EXIT_CODE})"
fi
echo "  Output: ${OUTPUT_BASE}"
echo "══════════════════════════════════════════════════════════════"
