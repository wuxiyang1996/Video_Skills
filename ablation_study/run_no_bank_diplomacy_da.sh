...
#  (step_0017 of Qwen3-8B_20260327_062035) with NO skill bank,
#  evaluated in 7-power Diplomacy against GPT-5.4 opponents.
...
#    bash ablation_study/run_no_bank_diplomacy_da.sh
#    EVAL_GPUS=0 bash ablation_study/run_no_bank_diplomacy_da.sh
#    NO_SERVER=1 VLLM_BASE_URL=http://localhost:8025/v1 bash ablation_study/run_no_bank_diplomacy_da.sh
# ======================================================================
set -euo pipefail
...
RUN_DIR="${RUN_DIR:-${PROJECT_ROOT}/runs/Qwen3-8B_20260327_062035}"
BEST_STEP="step_0017"
...
if [ -z "${OPENROUTER_API_KEY:-}" ] && [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "[no-bank-diplomacy-da] ERROR: No API key found for GPT-5.4 opponents."
    echo "  Set OPENROUTER_API_KEY or OPENAI_API_KEY, or configure api_keys.py."
    exit 1
fi

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT_BASE="${OUTPUT_DIR:-${PROJECT_ROOT}/ablation_study/output/no_bank_diplomacy_da_${TIMESTAMP}}"
mkdir -p "${OUTPUT_BASE}"
...
cleanup() {
    set +e
    echo ""
    echo "[no-bank-diplomacy-da] Shutting down..."
    ...
    echo "[no-bank-diplomacy-da] Done."
}
trap cleanup EXIT INT TERM

if [ ! -f "${ADAPTER_PATH}/adapter_config.json" ]; then
    echo "[no-bank-diplomacy-da] ERROR: Co-evolution adapter not found at ${ADAPTER_PATH}"
...
if [ "${NO_SERVER}" = "0" ]; then
    echo "[no-bank-diplomacy-da] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    echo "[no-bank-diplomacy-da] vLLM PID=${VLLM_PID}, log at ${VLLM_LOG}"

    MAX_WAIT=600
    WAITED=0
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[no-bank-diplomacy-da] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[no-bank-diplomacy-da] ERROR: vLLM server exited unexpectedly. Check ${VLLM_LOG}"
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[no-bank-diplomacy-da] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[no-bank-diplomacy-da] Using existing vLLM server at ${VLLM_BASE_URL}"
...
echo ""
echo "[no-bank-diplomacy-da] Command:"
...
echo ""
echo "══════════════════════════════════════════════════════════════"
echo "[no-bank-diplomacy-da] Output: ${OUTPUT_BASE}"
echo "══════════════════════════════════════════════════════════════"
exit ${EXIT_CODE}
