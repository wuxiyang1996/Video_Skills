#!/usr/bin/env bash
# ======================================================================
#  Ablation: SFT LoRA + Best Skill Bank vs GPT-5.4 on Diplomacy
#
#  Uses the SFT cold-start LoRA adapter (action_taking) paired with
#  the BEST skill bank (step_0017) from the co-evolution run
...
#    bash ablation_study/run_sft_best_bank_diplomacy_da.sh
#    EVAL_GPUS=0 bash ablation_study/run_sft_best_bank_diplomacy_da.sh
#    NO_SERVER=1 VLLM_BASE_URL=http://localhost:8025/v1 bash ablation_study/run_sft_best_bank_diplomacy_da.sh
# ======================================================================
set -euo pipefail
...
if [ -z "${OPENROUTER_API_KEY:-}" ] && [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "[sft-best-bank-diplomacy-da] ERROR: No API key found for GPT-5.4 opponents."
    echo "  Set OPENROUTER_API_KEY or OPENAI_API_KEY, or configure api_keys.py."
    exit 1
fi

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT_BASE="${OUTPUT_DIR:-${PROJECT_ROOT}/ablation_study/output/sft_best_bank_diplomacy_da_${TIMESTAMP}}"
mkdir -p "${OUTPUT_BASE}"
...
cleanup() {
    set +e
    echo ""
    echo "[sft-best-bank-diplomacy-da] Shutting down..."
    ...
    echo "[sft-best-bank-diplomacy-da] Done."
}
trap cleanup EXIT INT TERM

if [ ! -f "${SFT_ADAPTER_PATH}/adapter_config.json" ]; then
    echo "[sft-best-bank-diplomacy-da] ERROR: SFT adapter not found at ${SFT_ADAPTER_PATH}"
...
for power in AUSTRIA ENGLAND FRANCE GERMANY ITALY RUSSIA TURKEY; do
    ...
    if [ ! -f "${pbank}" ]; then
        echo "[sft-best-bank-diplomacy-da] ERROR: Skill bank not found for ${power}: ${pbank}"
        exit 1
    fi
    BANK_FILES+=("${pbank}")
done
...
echo "══════════════════════════════════════════════════════════════"
echo "  Ablation: SFT LoRA + Best Skill Bank"
...
if [ "${NO_SERVER}" = "0" ]; then
    echo "[sft-best-bank-diplomacy-da] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    echo "[sft-best-bank-diplomacy-da] vLLM PID=${VLLM_PID}, log at ${VLLM_LOG}"

    MAX_WAIT=600
    WAITED=0
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[sft-best-bank-diplomacy-da] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[sft-best-bank-diplomacy-da] ERROR: vLLM server exited unexpectedly. Check ${VLLM_LOG}"
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[sft-best-bank-diplomacy-da] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[sft-best-bank-diplomacy-da] Using existing vLLM server at ${VLLM_BASE_URL}"
...
echo ""
echo "[sft-best-bank-diplomacy-da] Command:"
...
echo ""
echo "══════════════════════════════════════════════════════════════"
echo "[sft-best-bank-diplomacy-da] Output: ${OUTPUT_BASE}"
echo "══════════════════════════════════════════════════════════════"
exit ${EXIT_CODE}
