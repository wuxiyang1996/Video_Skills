#!/usr/bin/env bash
# ======================================================================
#  Ablation: SFT LoRA + First Skill Bank vs GPT-5.4 on Diplomacy
#
#  Uses the SFT cold-start LoRA adapter (action_taking) paired with
#  the FIRST skill bank (step_0000) from the co-evolution run
#  Qwen3-8B_20260327_062035, evaluated in 7-power Diplomacy against
#  GPT-5.4 opponents.  4 episodes per power (28 total).
#
#  Run: Qwen3-8B_20260327_062035 (first step = 0, cold-start bank)
#
#  Usage:
#    conda activate game-ai-agent
#    bash ablation_study/run_sft_first_bank_diplomacy_da.sh
#    EVAL_GPUS=0 bash ablation_study/run_sft_first_bank_diplomacy_da.sh
#    NO_SERVER=1 VLLM_BASE_URL=http://localhost:8025/v1 bash ablation_study/run_sft_first_bank_diplomacy_da.sh
# ======================================================================
set -euo pipefail
...
FIRST_STEP="step_0000"
BANKS_DIR="${RUN_DIR}/checkpoints/${FIRST_STEP}/banks/diplomacy"
...
if [ -z "${OPENROUTER_API_KEY:-}" ] && [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "[sft-first-bank-diplomacy-da] ERROR: No API key found for GPT-5.4 opponents."
    echo "  Set OPENROUTER_API_KEY or OPENAI_API_KEY, or configure api_keys.py."
    exit 1
fi

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT_BASE="${OUTPUT_DIR:-${PROJECT_ROOT}/ablation_study/output/sft_first_bank_diplomacy_da_${TIMESTAMP}}"
mkdir -p "${OUTPUT_BASE}"
...
cleanup() {
    set +e
    echo ""
    echo "[sft-first-bank-diplomacy-da] Shutting down..."
    ...
    echo "[sft-first-bank-diplomacy-da] Done."
}
trap cleanup EXIT INT TERM

if [ ! -f "${SFT_ADAPTER_PATH}/adapter_config.json" ]; then
    echo "[sft-first-bank-diplomacy-da] ERROR: SFT adapter not found at ${SFT_ADAPTER_PATH}"
...
for power in AUSTRIA ENGLAND FRANCE GERMANY ITALY RUSSIA TURKEY; do
    ...
    if [ ! -f "${pbank}" ]; then
        echo "[sft-first-bank-diplomacy-da] ERROR: Skill bank not found for ${power}: ${pbank}"
        exit 1
    fi
    BANK_FILES+=("${pbank}")
done
...
echo "══════════════════════════════════════════════════════════════"
echo "  Ablation: SFT LoRA + First Skill Bank"
echo "           vs ${OPPONENT_MODEL} on Diplomacy"
echo "══════════════════════════════════════════════════════════════"
echo ""
echo "  Base model:      ${BASE_MODEL}"
echo "  SFT adapter:     ${SFT_ADAPTER_NAME} -> ${SFT_ADAPTER_PATH}"
echo "  Skill bank:      ${COMBINED_BANK} (${COMBINED_COUNT} skills, from ${FIRST_STEP})"
...
if [ "${NO_SERVER}" = "0" ]; then
    echo "[sft-first-bank-diplomacy-da] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    echo "[sft-first-bank-diplomacy-da] vLLM PID=${VLLM_PID}, log at ${VLLM_LOG}"

    MAX_WAIT=600
    WAITED=0
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[sft-first-bank-diplomacy-da] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[sft-first-bank-diplomacy-da] ERROR: vLLM server exited unexpectedly. Check ${VLLM_LOG}"
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[sft-first-bank-diplomacy-da] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[sft-first-bank-diplomacy-da] Using existing vLLM server at ${VLLM_BASE_URL}"
...
echo ""
echo "[sft-first-bank-diplomacy-da] Command:"
...
echo ""
echo "══════════════════════════════════════════════════════════════"
echo "[sft-first-bank-diplomacy-da] Output: ${OUTPUT_BASE}"
echo "══════════════════════════════════════════════════════════════"
exit ${EXIT_CODE}
