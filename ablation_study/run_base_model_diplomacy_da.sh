#!/usr/bin/env bash
# ======================================================================
#  Ablation: Base Qwen3-8B (No LoRA, No Skill Bank) vs GPT-5.4
#            on Diplomacy
#
#  Pure baseline — the vanilla Qwen3-8B base model with NO LoRA adapter
#  and NO skill bank, evaluated as a Decision Agent in 7-power Diplomacy
...
#    bash ablation_study/run_base_model_diplomacy_da.sh
#    EVAL_GPUS=0 bash ablation_study/run_base_model_diplomacy_da.sh
#    NO_SERVER=1 VLLM_BASE_URL=http://localhost:8025/v1 bash ablation_study/run_base_model_diplomacy_da.sh
# ======================================================================
set -euo pipefail
...
if [ -z "${OPENROUTER_API_KEY:-}" ] && [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "[base-diplomacy-da] ERROR: No API key found for GPT-5.4 opponents."
    echo "  Set OPENROUTER_API_KEY or OPENAI_API_KEY, or configure api_keys.py."
    exit 1
fi

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT_BASE="${OUTPUT_DIR:-${PROJECT_ROOT}/ablation_study/output/base_model_diplomacy_da_${TIMESTAMP}}"
mkdir -p "${OUTPUT_BASE}"
...
cleanup() {
    set +e
    echo ""
    echo "[base-diplomacy-da] Shutting down..."
    ...
    echo "[base-diplomacy-da] Done."
}
trap cleanup EXIT INT TERM

echo "══════════════════════════════════════════════════════════════"
echo "  Ablation: Base Qwen3-8B (No LoRA, No Skill Bank)"
...
if [ "${NO_SERVER}" = "0" ]; then
    echo "[base-diplomacy-da] Launching vLLM server on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    echo "[base-diplomacy-da] vLLM PID=${VLLM_PID}, log at ${VLLM_LOG}"

    MAX_WAIT=600
    WAITED=0
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[base-diplomacy-da] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[base-diplomacy-da] ERROR: vLLM server exited unexpectedly. Check ${VLLM_LOG}"
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[base-diplomacy-da] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[base-diplomacy-da] Using existing vLLM server at ${VLLM_BASE_URL}"
fi
...
echo ""
echo "[base-diplomacy-da] Command:"
...
echo ""
echo "══════════════════════════════════════════════════════════════"
echo "[base-diplomacy-da] Output: ${OUTPUT_BASE}"
echo "══════════════════════════════════════════════════════════════"
exit ${EXIT_CODE}
