...
For **detailed environment usage** (programmatic API, engine state transitions, agent configuration, CLI evaluation), see [docs/ENV_USAGE.md](docs/ENV_USAGE.md).
...
**Watch evaluation rollouts in the browser:** If you run external rollout scripts (e.g. the `evaluation_evolver` tests in the codebase that use the dummy LLM agent), you can stream their state into this visualizer. Start the server and open the observe page for the game (`/avalon/observe` or `/diplomacy/observe`), then run the script with its visualizer option (e.g. `--visualizer` and optionally `--visualizer_url http://localhost:8000`). The script will POST state updates to `/api/push-state` so the same browser view shows the rollout live.
