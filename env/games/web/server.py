...
async def push_state(state: Dict[str, Any]):
    """
    Update game state from external rollout (e.g. evaluation_evolver tests) and broadcast to observers.
    Body should include at least: game ("avalon" | "diplomacy"), status ("running" | "finished" | "stopped").
    Other keys (phase, mission_id, round_id, leader, roles, map_svg, logs, round, etc.) are forwarded to the frontend.
    """
    game = state.get("game")
    if game not in ["avalon", "diplomacy"]:
        raise HTTPException(status_code=400, detail="state.game must be 'avalon' or 'diplomacy'")
    state_manager.reset()
    state_manager.set_mode("observe", None, game=game)
    state_manager.update_game_state(**state)
    await state_manager.broadcast_message(state_manager.format_game_state())
    return {"status": "ok", "message": "State pushed"}
