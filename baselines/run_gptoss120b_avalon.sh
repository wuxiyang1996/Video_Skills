#!/usr/bin/env bash
# GPT-OSS 120B baseline on Avalon (40 episodes = 8/player x 5) via OpenRouter
# Controlled player = GPT-OSS 120B, all opponents = GPT-5.4
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/output/gptoss120b_avalon_${TIMESTAMP}}"
mkdir -p "${OUTPUT_DIR}"

echo "══════════════════════════════════════════════════════════════"
echo "  GPT-OSS 120B Baseline — Avalon (${EPISODES} episodes = ${EPISODES_PER_PLAYER}/player x ${NUM_PLAYERS})"
echo "══════════════════════════════════════════════════════════════"
...
echo ""
echo "  GPT-OSS 120B Avalon Baseline COMPLETE — ${OUTPUT_DIR}"
