#!/usr/bin/env bash
# Gemini 3.1 Pro baseline on Diplomacy (56 episodes = 8/power x 7) via OpenRouter
# Controlled power = Gemini 3.1 Pro, all opponents = GPT-5.4
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/output/gemini31pro_diplomacy_${TIMESTAMP}}"
mkdir -p "${OUTPUT_DIR}"

echo "══════════════════════════════════════════════════════════════"
echo "  Gemini 3.1 Pro Baseline — Diplomacy (${EPISODES} episodes = ${EPISODES_PER_POWER}/power x ${NUM_POWERS})"
echo "══════════════════════════════════════════════════════════════"
...
echo ""
echo "  Gemini 3.1 Pro Diplomacy Baseline COMPLETE — ${OUTPUT_DIR}"
