#!/usr/bin/env bash
# ======================================================================
#  Inference: Super Mario with best-performing Qwen3-8B checkpoint (8 episodes)
#
#  Checkpoint: runs/Qwen3-8B_super_mario_20260323_030839/best/ (step 11)
#  Training mean_reward: 930.0  (max 1410, min 702)
#
#  Launches a vLLM server for Qwen/Qwen3-8B with the best action_taking
#  LoRA adapter enabled and runs 8 inference episodes on Super Mario
...
#
#  Prerequisites:
#    conda create -n orak-mario python=3.11
#    conda run -n orak-mario pip install gym-super-mario-bros nes-py
#
#  Usage:
...
#
#    # With overrides:
#    EPISODES=16 bash inference/infer_super_mario_best.sh
#    EVAL_GPUS=0 bash inference/infer_super_mario_best.sh
#    NO_SERVER=1 VLLM_BASE_URL=http://localhost:8023/v1 bash inference/infer_super_mario_best.sh
# ======================================================================
set -euo pipefail
...
if [ -z "${DISPLAY:-}" ]; then
    if command -v Xvfb &>/dev/null; then
        ...
        if ! pgrep -f "Xvfb ${XVFB_DISPLAY}" &>/dev/null; then
            echo "[infer_mario_best] Starting Xvfb on ${XVFB_DISPLAY}..."
...
BANK_PATH="${BEST_DIR}/banks/super_mario/skill_bank.jsonl"
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/output/infer_super_mario_best_${TIMESTAMP}}"
mkdir -p "${OUTPUT_DIR}"
...
cleanup() {
    set +e
    echo ""
    echo "[infer_mario_best] Shutting down..."
    ...
    echo "[infer_mario_best] Done."
}
trap cleanup EXIT INT TERM

# ── Print banner ─────────────────────────────────────────────────────
echo "══════════════════════════════════════════════════════════════"
echo "  Super Mario Inference: Qwen3-8B best checkpoint (step 11)"
echo "══════════════════════════════════════════════════════════════"
...
if [ "${NO_SERVER}" = "0" ]; then
    echo "[infer_mario_best] Launching vLLM server with LoRA on ${VLLM_HOST}:${VLLM_PORT}..."
    ...
    while [ ${WAITED} -lt ${MAX_WAIT} ]; do
        if curl -sf "http://${VLLM_HOST}:${VLLM_PORT}/health" >/dev/null 2>&1; then
            echo "[infer_mario_best] vLLM server ready (waited ${WAITED}s)."
            break
        fi
        if ! kill -0 "${VLLM_PID}" 2>/dev/null; then
            echo "[infer_mario_best] ERROR: vLLM server exited unexpectedly."
            exit 1
        fi
        sleep 5
        WAITED=$((WAITED + 5))
    ...
    if [ ${WAITED} -ge ${MAX_WAIT} ]; then
        echo "[infer_mario_best] ERROR: vLLM server did not start within ${MAX_WAIT}s."
        exit 1
    fi
else
    echo "[infer_mario_best] Using existing vLLM server at ${VLLM_BASE_URL}"
fi
...
if [ ${EXIT_CODE} -eq 0 ]; then
    echo "  Super Mario best-checkpoint inference COMPLETE"
else
    echo "  Super Mario best-checkpoint inference FAILED (exit code ${EXIT_CODE})"
fi
echo "  Output: ${OUTPUT_DIR}"
echo "══════════════════════════════════════════════════════════════"
