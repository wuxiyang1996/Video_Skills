...
| [`m3bench_tool_calling_plan.md`](m3bench_tool_calling_plan.md) | The **M3-Bench-specific tool surface**: 8 typed tools (retrieval / perception / decision) that materialize the `[Search]` and `[Answer]` actions for M3-Bench, an **OpenAI-style function-calling protocol** to replace m3-agent's `Action: [Search]` text format, the **question-type → tool-chain** mapping, the **supervision signals** in `qa.reasoning` / `qa.timestamp` / `qa.before_clip`, **M3-Bench-specific anti-hacking constraints** (temporal leak, name leak, verifier collusion), and the open-work delta against the Phase-1 freeze-line. Strictly a concretization of `actors_reasoning_model.md`; introduces no new orchestration layer or memory store. | When you wire the M3-Bench adapter, port the m3-agent control loop onto this repo's controller, design M3-Bench SFT/GRPO trajectories, or add the `name_space_consistency` verifier check. |
